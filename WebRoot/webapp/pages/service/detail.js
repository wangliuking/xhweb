    ...
    pageview.init = function() {
     ...
     // 静态自定义绑定
        ...
        $("#progress").click(function() {
        	router.load({ url: "pages/service/progress.html",param:params});
        });
     // 自定义确定按钮事件
...
function sureBtn(){
	$.ajax({
		url : xh.getUrl()+'qualitycheck/checkedOne',
		type : 'POST',
		dataType : "json",
		async : true,
		data : {
		...
		success : function(data) {
			...
			if (data.success) {
				...
				bui.back({
					callback:function(){
						loader.require(["pages/service/table"],function(res){
							res.refresh();
							res.init();
	                    })
					}
...
function uploadFile() {
     ...
     if ($("#check-dg2").find('input[name="filePath"]').val() =="") {
      ...
      $.ajaxFileUpload({
          url : xh.getUrl()+'qualitycheck/upload', // 用于文件上传的服务器端请求地址
...
function uploadRecoreBtn(){
	$.ajax({
		url : xh.getUrl()+'qualitycheck/checkedTwo',
		type : 'POST',
		dataType : "json",
		async : true,
		data : {

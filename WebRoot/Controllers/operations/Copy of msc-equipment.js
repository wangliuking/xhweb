...
xh.load = function() {
	...
	app.controller("xhcontroller", function($scope, $http) {
		...
		$scope.drawCanvas=function(){
			scene.clear();
			var rootNode =xh.createRootNode(400, 400, 30, 30,"交换中心","tree");
            var dispatchNode =xh.createDispatchNode(800, 250, 30, 30,"调度台","circle");
			...
			success(function(response){
				...
				var totals = response.totals;
				 for(var j=0; j<totals; j++){
	                  var vmNode = new JTopo.Node(data[j].User_Unit);
	                  /*vmNode.radius = 10;
	                  vmNode.fillStyle = '255,255,0';*/
	                  vmNode.index=j;
	                  vmNode.setLocation(scene.width * Math.random(), scene.height * Math.random());
	                  vmNode.setImage("../../Resources/images/top/user-point.png",true);
	                  if(data[j].status!=1){
	                	  vmNode.alarm = '接入告警';
	                  }
	                  scene.add(vmNode); 
	                  var link=new JTopo.Link(accessNode, vmNode);
	                  link.lineWidth = 1; // 线宽
	                  link.bundleOffset = 60; // 折线拐角处的长度
	                  link.bundleGap = 20; // 线条之间的间隔
	                  if(data[j].status==1){
	                  ...
	                  JTopo.layout.layoutNode(scene, accessNode, true);
	            }

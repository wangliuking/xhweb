...
xh.load = function() {
	...
	app.controller("userstatus", function($scope, $http, $location) {
		...
		$scope.bsHover=function(index){
			//$("#aside-right-bottom").fadeToggle("fast");
			
			
			if($scope.data[index].status==1 &&$scope.data[index].link==0){
				$(".bs-name").html($scope.data[index].bsId+"-"+$scope.data[index].name);
				$(".bs-group").html($scope.data[index].groupSum);
				$(".bs-user").html($scope.data[index].userSum);
			}else{
				$(".bs-name").html($scope.data[index].bsId+"-"+$scope.data[index].name);
				$(".bs-group").html("--");
				$(".bs-user").html("--");
...
xh.excelToBsAlarm=function(){
	window.location.href="../../bsstatus/ExcelToBsAlarm";
};
//基站状态检查表
xh.excelToBsstatus=function(){
	window.location.href="../../bsstatus/ExcelToBsStatus";
};

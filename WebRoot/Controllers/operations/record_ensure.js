...
xh.load = function() {
	...
	app.controller("xhcontroller", function($scope, $http) {
		...
		//获取派单列表
		$http.get("../../userneed/communication_list?start=0&limit=" + pageSize).success(
				function(response) {
					xh.maskHide();
		...
		$scope.del=function(id){
			...
			$.ajax({
				url : '../../userneed/del_communication',
				type : 'post',
				dataType : "json",
				data : {
					id:$scope.oneData.id
		...
		$scope.search = function(page) {
			...
			xh.maskShow();
			$http.get("../../userneed/communication_list?start="+start+"&time="+starttime+"" +
		...
		$scope.pageClick = function(page, totals, totalPages) {
			...
			xh.maskShow();
			$http.get(
					"../../userneed/communication_list?start="+start+"&time="+starttime+"" +
					"&limit=" + pageSize).success(function(response) {
				xh.maskHide();
...
xh.add = function() {
	var $scope = angular.element(appElement).scope();
	$.ajax({
		url : '../../userneed/add_communication',
		type : 'POST',
		dataType : "json",
		async : true,
...
xh.edit = function() {
	var $scope = angular.element(appElement).scope();
	$.ajax({
		url : '../../userneed/update_communication',
		type : 'POST',
		dataType : "json",
		async : true,

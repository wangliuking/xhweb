...
xh.load = function() {
	...
	app.controller("xhcontroller", function($scope,$http) {
		...
		/*获取记录表*/
		$http.get("../../business/update_status_apply_list?start=0&limit=" + pageSize).
		...
		$scope.showCreateHtml=function(){
			window.location.href="asset_update_status_create.html";
		}
		$scope.toCheck = function (id,user,fileName,comment,applyTag) {
	    	window.location.href="asset_update_status_check.html?id" +
	    			"="+id+"&user="+user+"&fileName="+fileName+"&comment="+comment+"&applyTag="+applyTag;
	    };
		...
		$scope.toProgress = function (id) {
			...
			$scope.update_status_info_list(id);
			$("#progress").modal('show');
	    };
	    
		$scope.update_status_info_list = function(index) {
			...
			success(function(response){
				...
				$scope.update_status_info_totals = response.totals;
	    ...
	    $scope.check3 =function(index){
	    	var data=$scope.data[index];
	    	$.ajax({
	    		url : '../../business/asset/update_status_check3',
	    		type : 'POST',
	    		dataType : "json",
	    		async : false,
	    		data:{
		...
		$scope.search = function(page) {
			...
			xh.maskShow();
			$http.get("../../business/update_status_apply_list?start="+start+"&limit=" + pageSize).
			success(function(response){
				xh.maskHide();
				$scope.data = response.items;
				$scope.totals = response.totals;
...
xh.check2 = function(){
	...
	$.ajax({
		url : '../../business/asset/update_status_check2',
		type : 'POST',
		dataType : "json",
		async : false,
		data:{

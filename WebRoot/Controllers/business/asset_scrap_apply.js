...
xh.load = function() {
	...
	app.controller("xhcontroller", function($scope,$http) {
		...
		/*获取记录表*/
		$http.get("../../business/scrap_apply_list?start=0&limit=" + pageSize).
		...
		$scope.showCreateHtml=function(){
			window.location.href="asset_scrap_apply_create.html";
		}
		$scope.toCheck = function (id,user,fileName,comment,applyTag) {
	    	window.location.href="asset_scrap_apply_check.html?id" +
	    ...
	    $scope.check3 =function(index){
	    	var data=$scope.data[index];
	    	$.ajax({
	    		url : '../../business/asset/scrap_apply_check3',
	    		type : 'POST',
	    		dataType : "json",
	    		async : false,
	    		data:{
		...
		$scope.search = function(page) {
			...
			xh.maskShow();
			$http.get("../../business/scrap_apply_list?start="+start+"&limit=" + pageSize).
			success(function(response){
				xh.maskHide();
				$scope.data = response.items;
				$scope.totals = response.totals;
...
xh.check2 = function(){
	...
	$.ajax({
		url : '../../business/asset/scrap_apply_check2',
		type : 'POST',
		dataType : "json",
		async : false,
		data:{

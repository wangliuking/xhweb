...
xh.load = function() {
	...
	app.controller("xhcontroller", function($scope,$http, $location) {
		...
		$scope.doc_name+=$scope.time.split("-")[0]+$scope.time.split("-")[1]+"月考服务扣款表";
		...
		$scope.searchMoney=function(time){
			...
			success(function(response){
				...
				if(JSON.stringify($scope.money_data4).length<=2){
		...
		$scope.autoMoney=function(time){
			$http.get("../../check/show_money_detail?period=4&time="+time).
			success(function(response){
				xh.maskHide();
				$scope.money_data4 = response.items;
				$scope.money_sum4=response.sum;
			});
		}
	...
	$.ajax({
		...
		success : function(data) {
			if (data.success) {
				...
				POBrowser.openWindowModeless(xh.getUrl()+'/Views/jsp/check_create_money_doc.jsp','width=300px;height=200px;');
			} else {

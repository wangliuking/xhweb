...
xh.load = function() {
	...
	app.controller("talkgroupia", function($scope, $http) {
		xh.maskShow();
		$scope.count = "20";// 每页数据显示默认值
		$scope.systemMenu = true; // 菜单变色
		$http.get(
				"../../talkgroupia/list?start=0&limit=" + pageSize).success(
				function(response) {
					xh.maskHide();
		...
		$scope.search = function(page) {
			...
			xh.maskShow();
			$http.get(
					"../../talkgroupia/list?start=" + start + "&limit=" + limit).success(
					function(response) {
						xh.maskHide();
						$scope.data = response.items;
						$scope.totals = response.totals;
		...
		$scope.pageClick = function(page, totals, totalPages) {
			...
			xh.maskShow();
			$http.get(
					"../../talkgroupia/list?start=" + start + "&limit=" + pageSize).success(
					function(response) {
						xh.maskHide();
...
// 刷新数据
xh.refresh = function() {
	var appElement = document.querySelector('[ng-controller=talkgroupia]');

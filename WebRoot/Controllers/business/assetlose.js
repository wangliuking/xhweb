...
xh.load = function() {
	...
	app.controller("businesslose", function($scope, $http) {
		...
		$http.get("../../businesslose/assetList?type="+type+"&name="+name+"" +
				"&model="+model+"&serialNumber="+serialNumber+"&start=0&limit="+pageSize).
		...
		/* add页面搜索查询数据 */
		$scope.refreshlose = function() {
			$scope.searchlose(1);
			$("#table-checkbox-lose").prop("checked", false);
		};
		/* 显示修改model */
		...
		/* add页面查询数据 */
		$scope.searchlose = function(page) {
			var pageSize = $("#page-limit").val();
			var type = $("#typelose").val();
			var from = $("#fromlose").val();
			var status = $("#statuslose").val();
			var name = $("#namelose").val();
			var model = $("#modellose").val();
			...
			console.log("limit=" + limit);
			xh.maskShow();
			$http.get("../../business/assetList?type="+type+"&from="+from+"&status="+status+"&name="+name+"&model="+model+"&serialNumber="+serialNumber+"&start=0&limit="+pageSize).
			success(function(response){
				xh.maskHide();
				$scope.datatolose = response.items;
				$scope.totalslose = response.totals;
				xh.pagging(page, parseInt($scope.totals), $scope);
			});
		};
		//分页点击
		$scope.pageClick = function(page, totals, totalPages) {
			...
			xh.maskShow();
			$http.get("../../businesslose/assetList?type="+type+"&name="+name+"" +
					"&model="+model+"&serialNumber="+serialNumber+"&start="+start+"&limit="+pageSize).
			success(function(response){
				xh.maskHide();

...
/*	var type = $("#type").val();
	...
	app.controller("xhcontroller", function($scope,$http,$location) {
		...
		/*获取申请记录表*/
		$http.get("../../optimizenetdrop/selectAll?start=0&limit=" + pageSize).
		success(function(response){
			xh.maskHide();
		...
		$scope.search = function(page) {
			...
			xh.maskShow();
			$http.get("../../optimizenetdrop/selectAll?start=0&limit=" + limit).
			success(function(response){
				xh.maskHide();
				$scope.data = response.items;
				$scope.totals = response.totals;
		...
		$scope.pageClick = function(page, totals, totalPages) {
			...
			xh.maskShow();
			$http.get("../../optimizenetdrop/selectAll?start="+start+"&limit=" + limit).
			success(function(response){
				xh.maskHide();
...
xh.add = function() {
	$.ajax({
		url : '../../optimizenetdrop/insertOptimizeNetDrop',
		type : 'POST',
		dataType : "json",
		async : true,
		data:{
...
xh.check1 = function() {
	$.ajax({
		url : '../../optimizenetdrop/checkedOne',
		type : 'POST',
		dataType : "json",
		async : true,
...
xh.check2 = function() {
	$.ajax({
		url : '../../optimizenetdrop/checkedTwo',
		type : 'POST',
		dataType : "json",
		async : true,
...
xh.check3 = function() {
	$.ajax({
		url : '../../optimizenetdrop/checkedThree',
		type : 'POST',
		dataType : "json",
		async : true,
...
xh.check4 = function() {
	$.ajax({
		url : '../../optimizenetdrop/checkedFour',
		type : 'POST',
		dataType : "json",
		async : true,
...
xh.check5 = function() {
	$.ajax({
		url : '../../optimizenetdrop/checkedFive',
		type : 'POST',
		dataType : "json",
		async : true,
...
xh.upload = function(index) {
	...
	xh.maskShow();
	$.ajaxFileUpload({
		url : '../../optimizenetdrop/upload', // 用于文件上传的服务器端请求地址
...
xh.download=function(){
	...
	var downUrl = "../../optimizenetdrop/download?fileName=" + filename;

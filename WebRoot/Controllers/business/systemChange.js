...
/*	var type = $("#type").val();
	...
	app.controller("xhcontroller", function($scope,$http,$location) {
		...
		/*获取申请记录表*/
		$http.get("../../systemChange/selectAll?start=0&limit=" + pageSize).
		success(function(response){
			xh.maskHide();
        ...
        $scope.openWord = function(tag,fileName) {
            ...
            if(tag==1){
                ...
                POBrowser.openWindowModeless('../../systemChange/seal?type=1&path='+filepath,'width=1200px;height=800px;');
            }else if(tag==3){
                POBrowser.openWindowModeless('../../systemChange/seal?type=2&path='+filepath,'width=1200px;height=800px;');
            }else if(tag==4){
		...
		$scope.search = function(page) {
			...
			xh.maskShow();
			$http.get("../../systemChange/selectAll?start=0&limit=" + limit).
			success(function(response){
				xh.maskHide();
				$scope.data = response.items;
				$scope.totals = response.totals;
		...
		$scope.pageClick = function(page, totals, totalPages) {
			...
			xh.maskShow();
			$http.get("../../systemChange/selectAll?start="+start+"&limit=" + limit).
			success(function(response){
				xh.maskHide();
...
xh.check7 = function() {
    $.ajax({
        url : '../../systemChange/checkedSenven',
        type : 'POST',
        dataType : "json",
        async : true,
...
xh.checkNegFour = function() {
    $.ajax({
        url : '../../systemChange/checkedNegFour',
        type : 'POST',
        dataType : "json",
        async : true,
        data:$("#checkForm-4").serializeArray(),
        success : function(data) {
...
xh.upload = function(index) {
	...
	xh.maskShow();
	$.ajaxFileUpload({
		url : '../../systemChange/upload', // 用于文件上传的服务器端请求地址

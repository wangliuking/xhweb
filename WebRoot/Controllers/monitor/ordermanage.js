...
/*  var type = $("#type").val();
    ...
    app.controller("xhcontroller", function($scope,$http,$location) {
        ...
        /*获取故障记录表*/
        $http.get("../../ordermanage/selectAll?start=0&limit=" + pageSize).
        success(function(response){
            xh.maskHide();
        ...
        /*获取值班人员列表*/
        $http.get("../../web/user/getUserList?roleId=1001").
        success(function(response){
        ...
        $scope.checkWin = function (id) {
            ...
            $http.get("../../web/user/getUserList?roleId=1002").
            success(function(response){
            ...
            if($scope.loginUserRoleId==1001 && $scope.checkData.checked==0){
                $("#checkWin1").modal('show');
            }
            if($scope.loginUserRoleId==1002 && $scope.checkData.checked==1){
                $("#checkWin2").modal('show');
            }
            if($scope.loginUserRoleId==1004 && $scope.checkData.checked==2){
                $("#checkWin3").modal('show');
            }
            if($scope.loginUserRoleId==1001 && $scope.checkData.checked==3){
                $("#checkWin3").modal('show');
            }
        };
        /* 显示修改model */
        ...
        $scope.search = function(page) {
            ...
            xh.maskShow();
            $http.get("../../ordermanage/selectAll?start=0&limit=" + limit).
            success(function(response){
                xh.maskHide();
                $scope.data = response.items;
                $scope.totals = response.totals;
        ...
        $scope.pageClick = function(page, totals, totalPages) {
            ...
            xh.maskShow();
            $http.get("../../ordermanage/selectAll?start="+start+"&limit=" + limit).
            success(function(response){
                xh.maskHide();
...
//对应
/*故障申请*/
xh.addOrderManage = function() {
    $.ajax({
        url : '../../ordermanage/insertOrderManage',
        type : 'POST',
        dataType : "json",
        async : true,
        data:{
...
xh.download=function(){
    ...
    var downUrl="../../ordermanage/download?fileName="+filename;

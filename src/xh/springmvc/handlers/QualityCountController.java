...
import xh.mybatis.service.QualityCountService;

@Controller
@RequestMapping(value = "/qualitycount")
public class QualityCountController {
	private boolean success;
	private String message;
	private FunUtil funUtil = new FunUtil();
	protected final Log log = LogFactory.getLog(QualityCountController.class);
	...
	public void info(HttpServletRequest request, HttpServletResponse response) {
		...
		result.put("totals", QualityCountService.Count(map));
		result.put("items", QualityCountService.radioUserBusinessInfo(map));
	 ...
	 */
	@RequestMapping("/add")
	public void insertRadioUser(QualityCount record, HttpServletRequest request,
			HttpServletResponse response) {
		this.success = true;
		// 获取当前时间
		Date d = new Date();
		...
		record.setStatus(0);
		int count = QualityCountService.insert(record);
		HashMap result = new HashMap();
		result.put("success", success);
